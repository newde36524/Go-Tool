...
// OptimisticLock 乐观锁
func OptimisticLock[T any](ctx context.Context, conn sqlx.SqlConn, b squirrel.SelectBuilder, fn func(ctx context.Context, session sqlx.Session, in *T) error) error {
	data, err := Get[T](ctx, conn, b)
	if err != nil {
		return err
	}
