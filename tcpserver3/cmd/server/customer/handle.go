...
//ReadPacket .
func (TCPHandle) ReadPacket(conn *srv.Conn, next func()) srv.Packet {
	//todo 定义读取数据帧的规则
	b := make([]byte, 1024)
	n, err := conn.Read(b)
	if err != nil {
...
//OnConnection .
func (TCPHandle) OnConnection(conn *srv.Conn, next func()) {
	//todo 连接建立时处理,用于一些建立连接时,需要主动下发数据包的场景,可以在这里开启心跳协程,做登录验证等等
	logs.Infof("%s: 对方好像对你很感兴趣呦", conn.RemoteAddr())
}

//OnMessage .
func (TCPHandle) OnMessage(conn *srv.Conn, p srv.Packet, next func()) {
...
//OnClose .
func (TCPHandle) OnClose(state srv.ConnState, next func()) {
	logs.Infof("对方好像撤退了呦~~,连接状态:%s", state.String())
}

//OnTimeOut .
func (TCPHandle) OnTimeOut(conn *srv.Conn, code srv.TimeOutState, next func()) {
	logs.Infof("%s: 对方好像在做一些灰暗的事情呢~~,超时类型:%d", conn.RemoteAddr(), code)
}

//OnPanic .
func (TCPHandle) OnPanic(conn *srv.Conn, err error, next func()) {
	logs.Errorf("%s: 对方好像发生了一些不得了的事情哦~~,错误信息:%s", conn.RemoteAddr(), err)
}

//OnSendError .
func (TCPHandle) OnSendError(conn *srv.Conn, packet srv.Packet, err error, next func()) {
	logs.Errorf("%s: 发送数据的时间好像有点久诶~~,错误信息:%s", conn.RemoteAddr(), err)
}

//OnRecvError .
func (TCPHandle) OnRecvError(conn *srv.Conn, err error, next func()) {
